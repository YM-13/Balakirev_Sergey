...
class GamePole:
	...
	def init(self):
		...
		while m < self._m:
			...
			if self.pole[i][j].mine: # перейдет на continue только, если mine == True. По умолчанию mine == False
				continue
		...
		for x in range(self._n):
			for y in range(self._n):
				if not self.pole[x][y].mine:
					mines = sum((self.pole[x + i][y + j].mine for i, j in index if 0 <= x+i < self._n and 0 <= y+j < self._n))
